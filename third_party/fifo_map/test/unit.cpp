...
const auto collect_keys = [](const fifo_map<string, int>& m)
{
    string result;
    for (auto x : m)
    {
...
TEST_CASE("types")
{
    ...
    using fmap = fifo_map<string, int>;
...
{
    fifo_map<string, int> m = {{"C", 1}, {"A", 2}, {"B", 3}};
    const fifo_map<string, int> mc = m;
...
TEST_CASE("iterators")
{
    ...
    {
        fifo_map<string, int>::iterator it = m.begin();
        CHECK(it->first == "C");
        CHECK(it->second == 1);
        ++it;
        CHECK(it->first == "A");
    ...
    {
        fifo_map<string, int>::const_iterator it = mc.begin();
        CHECK(it->first == "C");
        CHECK(it->second == 1);
        ++it;
        CHECK(it->first == "A");
    ...
    {
        fifo_map<string, int>::const_iterator it = m.cbegin();
        CHECK(it->first == "C");
        CHECK(it->second == 1);
        ++it;
        CHECK(it->first == "A");
    ...
    {
        fifo_map<string, int>::const_iterator it = mc.cbegin();
        CHECK(it->first == "C");
        CHECK(it->second == 1);
        ++it;
        CHECK(it->first == "A");
    ...
    {
        fifo_map<string, int>::reverse_iterator it = m.rbegin();
        CHECK(it->first == "B");
        CHECK(it->second == 3);
        ++it;
        CHECK(it->first == "A");
    ...
    {
        fifo_map<string, int>::const_reverse_iterator it = mc.rbegin();
        CHECK(it->first == "B");
        CHECK(it->second == 3);
        ++it;
        CHECK(it->first == "A");
    ...
    {
        fifo_map<string, int>::const_reverse_iterator it = m.crbegin();
        CHECK(it->first == "B");
        CHECK(it->second == 3);
        ++it;
        CHECK(it->first == "A");
    ...
    {
        fifo_map<string, int>::const_reverse_iterator it = mc.crbegin();
        CHECK(it->first == "B");
        CHECK(it->second == 3);
        ++it;
        CHECK(it->first == "A");
...
TEST_CASE("capacity")
{
    nlohmann::fifo_map<string, int> m_empty;
    nlohmann::fifo_map<string, int> m_filled = {{"A", 1}, {"B", 2}};
...
{
    nlohmann::fifo_map<string, int> m_empty;
    nlohmann::fifo_map<string, int> m_filled = {{"X", 1}, {"C", 2}};
    ...
    SUBCASE("insert")
    {
        SUBCASE("insert value_type (lvalue)")
        {
            ...
            const nlohmann::fifo_map<string, int>::value_type v1 = {"A", 3};
            ...
            const nlohmann::fifo_map<string, int>::value_type v2 = {"A", 4};
        ...
        SUBCASE("insert range")
        {
            ...
            std::map<string, int> values = {{"A", 3}, {"Z", 4}, {"B", 5}};
...
TEST_CASE("lookup")
{
    nlohmann::fifo_map<string, int> m = {{"A", 1}, {"B", 2}};
    const nlohmann::fifo_map<string, int> m_c = m;
    ...
    SUBCASE("equal_range")
    {
        ...
        {
            {
                std::pair<fifo_map<string, int>::iterator, fifo_map<string, int>::iterator> range =
                    m.equal_range("A");
        ...
        {
            {
                std::pair<fifo_map<string, int>::const_iterator, fifo_map<string, int>::const_iterator>
    ...
    SUBCASE("lower_bound")
    {
        ...
        {
            {
                fifo_map<string, int>::iterator it = m.lower_bound("A");
                CHECK(it == m.begin());
            }
            {
                fifo_map<string, int>::iterator it = m.lower_bound("C");
                CHECK(it == m.end());
            }
        }
        ...
        {
            {
                fifo_map<string, int>::const_iterator it = m_c.lower_bound("A");
                CHECK(it == m_c.cbegin());
            }
            {
                fifo_map<string, int>::const_iterator it = m_c.lower_bound("C");
                CHECK(it == m_c.cend());
            }
        }
    }
    ...
    SUBCASE("upper_bound")
    {
        ...
        {
            {
                fifo_map<string, int>::iterator it = m.upper_bound("A");
                CHECK(it == ++m.begin());
            }
            {
                fifo_map<string, int>::iterator it = m.upper_bound("C");
                CHECK(it == m.end());
            }
        }
        ...
        {
            {
                fifo_map<string, int>::const_iterator it = m_c.upper_bound("A");
                CHECK(it == ++m_c.cbegin());
            }
            {
                fifo_map<string, int>::const_iterator it = m_c.upper_bound("C");
                CHECK(it == m_c.cend());
            }
        }
    }
...
{
    nlohmann::fifo_map<string, int> m1 = {{"A", 1}, {"B", 2}};
    nlohmann::fifo_map<string, int> m2 = {{"B", 2}, {"A", 1}};
    nlohmann::fifo_map<string, int> m3 = {{"A", 3}, {"B", 4}};
...
TEST_CASE("regression tests")
{
    ...
    {
        nlohmann::fifo_map<string, uint8_t> pe;
...
TEST_CASE("constructors")
{
    ...
    {
        std::map<string, int> inputs = {{"A", 1}, {"B", 2}};
        nlohmann::fifo_map<string, int> m(inputs.begin(), inputs.end());
        CHECK(m.size() == 2);
        CHECK(m["A"] == 1);
        CHECK(m["B"] == 2);
