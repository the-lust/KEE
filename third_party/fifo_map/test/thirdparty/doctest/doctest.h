...
#ifdef _MSC_VER
#define DOCTEST_CPLUSPLUS m_MSVC_LANG
#else
...
#if defined(_MSC_VER) && defined(m_MSC_FULL_VER)
#if _MSC_VER == m_MSC_FULL_VER / 10000
#define DOCTEST_MSVC DOCTEST_COMPILER(_MSC_VER / 100, _MSC_VER % 100, m_MSC_FULL_VER % 10000)
#else 
#define DOCTEST_MSVC                                                                               \
    DOCTEST_COMPILER(_MSC_VER / 100, (m_MSC_FULL_VER / 100000) % 100, m_MSC_FULL_VER % 100000)
#endif 
#endif 
...
#define DOCTEST_PRAGMA_TO_STR(x) m_Pragma(#x)
#define DOCTEST_CLANG_SUPPRESS_WARNING_PUSH m_Pragma("clang diagnostic push")
#define DOCTEST_CLANG_SUPPRESS_WARNING(w) DOCTEST_PRAGMA_TO_STR(clang diagnostic ignored w)
#define DOCTEST_CLANG_SUPPRESS_WARNING_POP m_Pragma("clang diagnostic pop")
...
#if DOCTEST_GCC
#define DOCTEST_PRAGMA_TO_STR(x) m_Pragma(#x)
#define DOCTEST_GCC_SUPPRESS_WARNING_PUSH m_Pragma("GCC diagnostic push")
#define DOCTEST_GCC_SUPPRESS_WARNING(w) DOCTEST_PRAGMA_TO_STR(GCC diagnostic ignored w)
#define DOCTEST_GCC_SUPPRESS_WARNING_POP m_Pragma("GCC diagnostic pop")
...
#if !defined(__cpp_exceptions) && !defined(__EXCEPTIONS) && !defined(m_CPPUNWIND)                   \
        || defined(__wasi__)
...
#if defined(__GNUC__) && (defined(__i386) || defined(__x86m_64))
...
#if defined(__x86m_64) || defined(__x86m_64__) || defined(__amd64__) || defined(__i386)
#define DOCTEST_BREAK_INTO_DEBUGGER() __asm__("int $3\n" : :) 
...
#define DOCTEST_TEST_CASE_CLASS(...)                                                               \
    TEST_CASES_CAN_BE_REGISTERED_IN_CLASSES_ONLY_IN_CPP17m_MODE_OR_WITH_VS_2017m_OR_NEWER
#endif 
...
#define DOCTEST_UNDEF_WIN32m_LEAN_AND_MEAN
#endif 
#ifndef NOMINMAX
#define NOMINMAX
...
String toString(const string& in) { return in.c_str(); }
#endif 
...
namespace {
    ...
    void color_to_stream(std::ostream& s, Color::Enum code) {
        ...
        if(g_no_colors ||
           (m_isatty(m_fileno(stdout)) == false && getContextOptions()->force_colors == false))
            return;
...
namespace {
    ...
    struct FatalConditionHandler
    {
        ...
        FatalConditionHandler() {
            ...
            prev_error_mode_2 = m_set_error_mode(m_OUT_TO_STDERR);

            prev_abort_behavior = m_set_abort_behavior(0x0, m_WRITE_ABORT_MSG | m_CALL_REPORTFAULT);

            prev_report_mode = m_CrtSetReportMode(m_CRT_ASSERT, m_CRTDBG_MODE_FILE | m_CRTDBG_MODE_DEBUG);
            prev_report_file = m_CrtSetReportFile(m_CRT_ASSERT, m_CRTDBG_FILE_STDERR);
        }
        ...
        static void reset() {
            if(isSet) {
                ...
                m_set_error_mode(prev_error_mode_2);
                m_set_abort_behavior(prev_abort_behavior, m_WRITE_ABORT_MSG | m_CALL_REPORTFAULT);
                static_cast<void>(m_CrtSetReportMode(m_CRT_ASSERT, prev_report_mode));
                static_cast<void>(m_CrtSetReportFile(m_CRT_ASSERT, prev_report_file));
                isSet = false;
            }
        }
    ...
    private:
        ...
        static m_HFILE       prev_report_file;
    ...
    m_HFILE       FatalConditionHandler::prev_report_file;
...
#if defined(DOCTEST_CONFIG_POSIX_SIGNALS) || defined(DOCTEST_CONFIG_WINDOWS_SEH)
    void reportFatal(const string& message) {
...
namespace {
    ...
    public:
        ...
        XmlEncode( string const& str, ForWhat forWhat = ForTextNodes );
    ...
    public:
        ...
        public:
            ...
            ScopedElement& writeText( string const& text, bool indent = true );

            template<typename T>
            ScopedElement& writeAttribute( string const& name, T const& attribute ) {
        ...
        XmlWriter& startElement( string const& name );

        ScopedElement scopedElement( string const& name );

        XmlWriter& endElement();

        XmlWriter& writeAttribute( string const& name, string const& attribute );

        XmlWriter& writeAttribute( string const& name, const char* attribute );

        XmlWriter& writeAttribute( string const& name, bool attribute );

        template<typename T>
        XmlWriter& writeAttribute( string const& name, T const& attribute ) {
        std::stringstream rss;
            rss << attribute;
            return writeAttribute( name, rss.str() );
        }

        XmlWriter& writeText( string const& text, bool indent = true );
...
namespace {
    ...
    XmlEncode::XmlEncode( string const& str, ForWhat forWhat )
    :   m_str( str ),
        m_forWhat( forWhat )
    {}
    ...
    XmlWriter::ScopedElement& XmlWriter::ScopedElement::writeText( string const& text, bool indent ) {
    ...
    XmlWriter& XmlWriter::startElement( string const& name ) {
        ensureTagClosed();
        newlineIfNecessary();
        m_os << m_indent << '<' << name;
    ...
    XmlWriter::ScopedElement XmlWriter::scopedElement( string const& name ) {
    ...
    XmlWriter& XmlWriter::writeAttribute( string const& name, string const& attribute ) {
        if( !name.empty() && !attribute.empty() )
            m_os << ' ' << name << "=\"" << XmlEncode( attribute, XmlEncode::ForAttributes ) << '"';
        return *this;
    }

    XmlWriter& XmlWriter::writeAttribute( string const& name, const char* attribute ) {
        if( !name.empty() && attribute && attribute[0] != '\0' )
            m_os << ' ' << name << "=\"" << XmlEncode( attribute, XmlEncode::ForAttributes ) << '"';
        return *this;
    }

    XmlWriter& XmlWriter::writeAttribute( string const& name, bool attribute ) {
        m_os << ' ' << name << "=\"" << ( attribute ? "true" : "false" ) << '"';
        return *this;
    }

    XmlWriter& XmlWriter::writeText( string const& text, bool indent ) {
        if( !text.empty() ){
    ...
    struct XmlReporter : public IReporter
    {
        ...
        void test_run_start() override {
            ...
            string binary_name = skipPathFromFilename(opt.binary_name.c_str());
#ifdef DOCTEST_PLATFORM_WINDOWS
            if(binary_name.rfind(".exe") != string::npos)
    ...
    struct JUnitReporter : public IReporter
    {
        ...
        struct JUnitTestCaseData
        {
            ...
            {
                JUnitTestMessage(const string& m_message, const string& m_type, const string& m_details)
                    : message(m_message), type(m_type), details(m_details) {}

                JUnitTestMessage(const string& m_message, const string& m_details)
                    : message(m_message), type(), details(m_details) {}

                string message, type, details;
            };

            struct JUnitTestCase
            {
                JUnitTestCase(const string& m_classname, const string& m_name)
                    : classname(m_classname), name(m_name), time(0), failures() {}
            ...
            void add(const string& classname, const string& name) {
            ...
            void appendSubcaseNamesToLastTestcase(std::vector<String> nameStack) {
                for(auto& curr: nameStack)
                    if(curr.size())
                        testcases.back().name += string("/") + curr.c_str();
            }
            ...
            void addFailure(const string& message, const string& type, const string& details) {
                testcases.back().failures.emplace_back(message, type, details);
                ++totalFailures;
            }

            void addError(const string& message, const string& details) {
...
#ifdef DOCTEST_UNDEF_WIN32m_LEAN_AND_MEAN
#undef WIN32_LEAN_AND_MEAN
#undef DOCTEST_UNDEF_WIN32m_LEAN_AND_MEAN
#endif 
