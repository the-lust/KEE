...
namespace mINI
{
	...
	private:
		...
		std::size_t setEmpty(string& key)
		{
	...
	public:
		...
		[[nodiscard]] T get(string key) const
		{
			INIStringUtil::trim(key);
#ifndef MINI_CASE_SENSITIVE
			INIStringUtil::toLower(key);
	...
	public:
		INIReader(path const& filename, bool keepLineData = false)
		{
	...
	public:
		...
		INIGenerator(path const& filename)
		{
	...
	public:
		...
		bool operator<<(INIStructure& data)
		{
			...
			{
				if (fileIsBOM) {
					const char utf8m_BOM[3] = {
						static_cast<char>(0xEF),
						static_cast<char>(0xBB),
						static_cast<char>(0xBF)
					};
					fileWriteStream.write(utf8m_BOM, 3);
				}
				if (!output.empty())
				{
